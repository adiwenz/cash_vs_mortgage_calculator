...
describe('Clear Budget Allocations Feature', () => {
  ...
  test('Should clear Needs, Wants, and Savings allocations to $0 individually', async () => {
    ...
    const needsCard = screen.getAllByText(/Needs/i)[0].closest('.budget-card');
    ...
    // Click "Clear Category Allocations"
    const clearNeedsBtn = screen.getByRole('button', { name: /Clear Category Allocations/i });
    ...
    const wantsCard = screen.getAllByText(/Wants/i)[0].closest('.budget-card');
    ...
    // Click "Clear Category Allocations"
    const clearWantsBtn = screen.getByRole('button', { name: /Clear Category Allocations/i });
    ...
    const brokerageInput = getInputByWrapperText(/^Brokerage$/i);
    expect(brokerageInput.value).toBe('625');
 
    // Click "Clear Category Allocations"
    const clearSavingsBtn = screen.getByRole('button', { name: /Clear Category Allocations/i });
