...
export default function CreditCardBehavior() {
  ...
  const [activeScenario, setActiveScenario] = useState('neutral'); // 'aggressive' | 'slow' | 'neutral' | 'trap' | 'budgetGap'
  ...
  const simulations = useMemo(() => {
    ...
    // Compute payments for scenarios
    const P_aggressive = Math.max(I + 150, monthlyPayment * 2.5);
    const P_slow = Math.max(I + 10, monthlyPayment * 1.25);
    const P_neutral = I;
    const P_trap = Math.max(10, Math.min(I - 10, monthlyPayment * 0.75));
    const P_budget = monthlyPayment;

    // Simulation outputs with extra payments
    const aggressive = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_aggressive, monthlyNewDebt: 0, extraPayment, startingCash });
    const slow = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_slow, monthlyNewDebt: 0, extraPayment, startingCash });
    const neutral = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_neutral, monthlyNewDebt: 0, extraPayment, startingCash });
    const trap = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_trap, monthlyNewDebt: 0, extraPayment, startingCash });
    const budgetGap = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_budget, monthlyNewDebt: monthlyNewDebt, extraPayment, startingCash });

    // Baseline outputs (no extra payment) to calculate before/after impact
    const aggressiveBase = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_aggressive, monthlyNewDebt: 0, extraPayment: 0, startingCash });
    const slowBase = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_slow, monthlyNewDebt: 0, extraPayment: 0, startingCash });
    const neutralBase = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_neutral, monthlyNewDebt: 0, extraPayment: 0, startingCash });
    const trapBase = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_trap, monthlyNewDebt: 0, extraPayment: 0, startingCash });
    const budgetGapBase = simulateDebt({ startingBalance: balance, apr: r, monthlyPayment: P_budget, monthlyNewDebt: monthlyNewDebt, extraPayment: 0, startingCash });

    return {
      aggressive: { current: aggressive, base: aggressiveBase, payment: P_aggressive + extraPayment, name: 'Aggressive Paydown' },
      slow: { current: slow, base: slowBase, payment: P_slow + extraPayment, name: 'Slow Paydown' },
      neutral: { current: neutral, base: neutralBase, payment: P_neutral + extraPayment, name: 'Interest Neutral' },
      trap: { current: trap, base: trapBase, payment: P_trap + extraPayment, name: 'Interest Trap' },
      budgetGap: { current: budgetGap, base: budgetGapBase, payment: P_budget + extraPayment, name: 'Budget Gap' }
    };
  }, [inputs]);
  ...
  const yAxisBoundsAt36 = useMemo(() => {
    ...
    const sims36 = [aggressive, slow, neutral, trap, budgetGap];
  ...
  }, [inputs.balance, inputs.monthlyPayment, inputs.monthlyNewDebt, inputs.extraPayment, inputs.startingCash]);

  // Determine line colors based on color-blind setting
  const getLineColors = (key) => {
    if (colorBlindMode) {
      switch (key) {
        case 'aggressive': return '#2563eb'; // Blue
        case 'slow': return '#0284c7';       // Sky Blue
        case 'neutral': return '#ea580c';    // Orange
        case 'trap': return '#dc2626';       // Bright Red
  ...
  const activeSim = simulations[activeScenario];
  ...
  }, [activeScenario, activeSim, inputs, monthlyInterest]);
  ...
  const formatPayoffSummary = (simObj) => {
    ...
    if (simObj.name === 'Budget Gap' && sim.yearlyData[30].balance > inputs.balance) {
      return `Grows to ${formatCurrency(sim.yearlyData[10].balance)} (10y)`;
    }
    if (sim.isPaidOff) {
  ...
  const interestSaved = useMemo(() => {
    const neutralBaseline = simulations.neutral.base.totalInterestPaid;
    const currentInterest = activeSim.current.totalInterestPaid;
    return Math.max(0, neutralBaseline - currentInterest);
  }, [simulations, activeSim, activeScenario]);
        ...
        <div className="results-display">
              ...
              return (
                ...
                >
                  <div className="scenario-card-header">
                    <span className="scenario-dot" style={{ backgroundColor: getLineColors(key) }} />
                    <span className="scenario-name-text">{simObj.name}</span>
                  </div>
                  <div className="scenario-timeline-val">
                    {formatPayoffSummary(simObj)}
                  </div>
          ...
          <div className="cc-metrics-dashboard">
            ...
            <div className="cc-metric-card border-emerald">
              ...
              <span className="cc-metric-subtitle">Compared to Interest Neutral</span>
            </div>
          </div>
            ...
            <div className="glass-card chart-card" id="cc-balance-chart-card">
              <div className="chart-header-row">
                <div>
                  ...
                  <span className="chart-subtitle">Comparing remaining debt balance profiles over 30 years</span>
                </div>
              ...
              <div className="chart-container-inner" style={{ height: '320px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Line
                      type="monotone"
                      dataKey="Aggressive Paydown"
                      stroke={getLineColors('aggressive')}
                      strokeWidth={activeScenario === 'aggressive' ? 3.5 : 1.5}
                      dot={false}
                      activeDot={{ r: 6 }}
                    />
                    <Line
                      type="monotone"
                      dataKey="Slow Paydown"
                      stroke={getLineColors('slow')}
                      strokeWidth={activeScenario === 'slow' ? 3.5 : 1.5}
                      dot={false}
                      activeDot={{ r: 6 }}
                    />
                    <Line
                      type="monotone"
                      dataKey="Interest Neutral"
                      stroke={getLineColors('neutral')}
                      strokeWidth={activeScenario === 'neutral' ? 3.5 : 1.5}
                      dot={false}
                      activeDot={{ r: 6 }}
                    />
                    <Line
                      type="monotone"
                      dataKey="Interest Trap"
                      stroke={getLineColors('trap')}
                      strokeWidth={activeScenario === 'trap' ? 3.5 : 1.5}
                      dot={false}
                      activeDot={{ r: 6 }}
                    />
                    <Line
                      type="monotone"
                      dataKey="Budget Gap"
                      stroke={getLineColors('budgetGap')}
                      strokeWidth={activeScenario === 'budgetGap' ? 3.5 : 1.5}
                      dot={false}
                      activeDot={{ r: 6 }}
                    />
                    <Line
          ...
          {/* Chart 2: Credit Card Drag Over Time (New) */}
          {activeTab === 'drag' && (
              ...
              <div className="chart-container-inner" style={{ height: '320px' }}>
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Line
                      type="monotone"
                      dataKey="Interest Neutral NW"
                      name="Interest Neutral"
                      stroke={getLineColors('neutral')}
                      strokeWidth={activeScenario === 'neutral' ? 3.5 : 1.5}
                      dot={false}
                      activeDot={{ r: 6 }}
          ...
          {/* Chart 3: Alternate Payoff Scenarios (New) */}
          {activeTab === 'payoff' && (
          ...
          <div className="glass-card active-insights-card" style={{ borderLeft: `5px solid ${getLineColors(activeScenario)}` }}>
            ...
            <div className="insights-content-layout">
              ...
              <div className="insights-metrics-panel">
                {activeScenario === 'aggressive' || activeScenario === 'slow' || activeScenario === 'neutral' ? (
                  <>
            ...
            <div className="insights-action-tips">
              ...
              <p className="tip-message">
                {activeScenario === 'aggressive' && "Paying extra is the fastest way to reduce principal. Every dollar added directly reduces the amount that interest accumulates on, halting wealth decay."}
                {activeScenario === 'slow' && "When payments only slightly exceed interest, interest compounds consume almost your entire payment. You end up renting your debt and leaking wealth for decades."}
                {activeScenario === 'neutral' && "This is a common trap. Paying only interest keeps the principal completely intact. You make payments, but your net worth falls continuously."}
                {activeScenario === 'trap' && "This occurs when minimum payments are set below interest charges. The debt expands automatically, eating into your future wealth."}
                {activeScenario === 'budgetGap' && "This is the most critical pattern to break. Adding new debt while carrying a balance creates a compounding cycle that escalates rapidly, destroying net worth."}
              </p>
            </div>
          </div>
