...
export default function RelationshipWizard({
  ...
  const isShortFlow = relationshipType === 'partner' || relationshipType === 'engaged';

  const handleNext = () => {
    if (stepId < 4) {
      const nextStep = (stepId === 1 && isShortFlow) ? 3 : stepId + 1;
      setEditingEvent({ ...editingEvent, wizardStep: nextStep });
    }
  };

  const handleBack = () => {
    if (stepId > 1) {
      const prevStep = (stepId === 3 && isShortFlow) ? 1 : stepId - 1;
      setEditingEvent({ ...editingEvent, wizardStep: prevStep });
    }
  };
  ...
  const isStep4Invalid = editingEvent.combineFinances !== false && (
    (combinedSpendingVal <= userSpendingPreRetirement && !isZeroSpendingConfirmed) || 
    (partnerPersonalSpending === 0 && !isPartnerZeroSpendingConfirmed)
  );
  ...
  const displayHeading = relationshipType === 'married' ? 'Get Married' : 
                         relationshipType === 'domestic_partnership' ? 'Domestic Partnership' : 
                         'Relationship';
  ...
  return (
    <div className="modal-backdrop" onClick={() => { setEditingEvent(null); setIsFullPartnerProfileOpen(false); setIsZeroSpendingConfirmed(false); setIsPartnerZeroSpendingConfirmed(false); }}>
      ...
      >
        ...
        {stepId > 1 && (
          <div className="wizard-steps-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem', marginBottom: '1.5rem', borderBottom: '1px solid var(--border-color)', paddingBottom: '1rem' }}>
            ...
            <div className={`wizard-step-node ${stepId === 1 ? 'active' : ''} ${stepId > 1 ? 'completed' : ''}`} onClick={() => setEditingEvent({ ...editingEvent, wizardStep: 1 })}>
              ...
              <span className="wizard-step-label" style={{ fontSize: '0.75rem' }}>Details</span>
            </div>
            ...
            <div className={`wizard-step-node ${stepId === 3 ? 'active' : ''} ${stepId > 3 ? 'completed' : ''}`} onClick={() => setEditingEvent({ ...editingEvent, wizardStep: 3 })}>
              <div className="wizard-step-icon">{isShortFlow ? '2' : '3'}</div>
            ...
            <div className={`wizard-step-node ${stepId === 4 ? 'active' : ''}`} onClick={() => {
              ...
              <div className="wizard-step-icon">{isShortFlow ? '3' : '4'}</div>
              <span className="wizard-step-label" style={{ fontSize: '0.75rem' }}>Impact</span>
            </div>
          </div>
        )}

        {/* STEP 1: CONGRATULATIONS & SETUP */}
        {stepId === 1 && (
          <div className="animate-slide-up" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <div style={{ position: 'relative', textAlign: 'center', padding: '0.5rem 0' }}>
              ...
              <h4 style={{
                ...
                {relationshipType === 'married' ? 'Congrats! 🎉' : 'Setup Relationship'}
              </h4>
              <p style={{ fontSize: '1.15rem', color: 'var(--text-secondary)', margin: 0, fontWeight: '500' }}>
                {editingEvent.combineFinances !== false 
                  ? "You'll combine finances for projections." 
            ...
            {/* Relationship setup fields */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '0.5rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                <div className="input-wrapper">
                  <span className="input-name">Partner Name</span>
                  <input
                    type="text"
                    className="input-number-box"
                    style={{ width: '100%' }}
                    value={editingEvent.spouseName || ''}
                    placeholder={relationshipType === 'married' ? 'Spouse' : 'Partner'}
                    onChange={(e) => setEditingEvent(Object.assign({}, editingEvent, { spouseName: e.target.value }))}
                  />
                </div>
                <div className="input-wrapper">
                  <span className="input-name">Relationship Type</span>
                  <select
                    className="input-number-box"
                    style={{ width: '100%', background: 'var(--bg-secondary)', color: 'var(--text-primary)', height: '36px', borderRadius: '6px', border: '1px solid var(--border-color)', padding: '0 0.5rem' }}
                    value={relationshipType}
                    onChange={(e) => {
                      const val = e.target.value;
                      const defaultType = val === 'married' ? 'marriage' : (val === 'domestic_partnership' ? 'domesticPartnership' : 'relationshipBegins');
                      setEditingEvent(Object.assign({}, editingEvent, {
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginTop: '0.25rem' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.85rem', color: 'var(--text-primary)' }}>
                  <input
                    type="checkbox"
                    checked={editingEvent.livingTogether !== false}
                    onChange={(e) => setEditingEvent(Object.assign({}, editingEvent, { livingTogether: e.target.checked }))}
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.85rem', color: 'var(--text-primary)' }}>
                  <input
                    type="checkbox"
                    checked={editingEvent.combineFinances !== false}
                    onChange={(e) => setEditingEvent(Object.assign({}, editingEvent, { combineFinances: e.target.checked }))}
                    style={{ width: '1rem', height: '1rem', cursor: 'pointer' }}
                  />
                  Combine finances
                </label>
            ...
            {isFullPartnerProfileOpen && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', padding: '1rem', background: 'rgba(255,255,255,0.01)', borderRadius: '8px', border: '1px solid var(--border-color)', marginTop: '0.5rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                  <div className="input-wrapper">
                    <span className="input-name">{relationshipType === 'married' ? 'Marriage Age' : 'Relationship Age'}</span>
                    <input
                      type="number"
                      className="input-number-box"
                      style={{ width: '100%' }}
                  ...
                  <div className="input-wrapper">
                    <span className="input-name">{relationshipType === 'married' ? 'Spouse Current Age' : 'Partner Current Age'}</span>
                    <input
                      type="number"
                      className="input-number-box"
                      style={{ width: '100%' }}
                  ...
                  <div className="input-wrapper">
                    <span className="input-name">{relationshipType === 'married' ? 'Spouse Income' : 'Partner Income'} ($/year)</span>
                    <CurrencyInput
                      className="input-number-box"
                      style={{ width: '100%' }}
                  ...
                  <div className="input-wrapper">
                    <span className="input-name">{relationshipType === 'married' ? 'Spouse Work Optional Age' : 'Partner Work Optional Age'}</span>
                    <input
                      type="number"
                      className="input-number-box"
                      style={{ width: '100%' }}
                  ...
                  <div className="input-wrapper">
                    <span className="input-name">{relationshipType === 'married' ? 'Spouse Life Expectancy' : 'Partner Life Expectancy'}</span>
                    <input
                      type="number"
                      className="input-number-box"
                      style={{ width: '100%' }}
                  ...
                  <div className="input-wrapper">
                    <span className="input-name">Partner SS Claiming Age</span>
                    <input
                      type="number"
                      className="input-number-box"
                      style={{ width: '100%' }}
                  ...
                  <div className="input-wrapper">
                    <span className="input-name">Partner SS Benefit ($/yr)</span>
                    <CurrencyInput
                      className="input-number-box"
                      style={{ width: '100%' }}
        ...
        {/* STEP 2: WEDDING */}
        {stepId === 2 && !isShortFlow && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            <div style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
              <h4 style={{ fontSize: '0.95rem', fontWeight: 'bold', color: 'var(--text-primary)', margin: '0 0 0.25rem 0' }}>{relationshipType === 'married' ? 'Plan Your Wedding' : 'Plan Your Celebration'}</h4>
              <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', margin: 0 }}>
                {relationshipType === 'married' ? 'Determine your wedding budget, evaluate funding sources, and identify any savings gaps.' : 'Determine your celebration budget, evaluate funding sources, and identify any savings gaps.'}
              </p>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <label htmlFor="include-wedding-cost" className="input-name" style={{ margin: 0, cursor: 'pointer' }}>
                {relationshipType === 'married' ? 'Plan to have a wedding' : 'Plan to have a celebration'}
              </label>
            </div>
            ...
            {(editingEvent.includeWeddingCost !== undefined ? !!editingEvent.includeWeddingCost : true) && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', padding: '0.75rem', background: 'rgba(255,255,255,0.01)', borderRadius: '6px', border: '1px solid var(--border-color)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                  <div className="input-wrapper">
                    <span className="input-name">{relationshipType === 'married' ? 'Wedding Cost ($)' : 'Celebration Cost ($)'}</span>
                    <CurrencyInput
                      className="input-number-box"
                      style={{ 
                        width: '100%',
                  ...
                  <div className="input-wrapper">
                    <span className="input-name">{relationshipType === 'married' ? 'Wedding Age' : 'Celebration Age'}</span>
                    <input
                      type="number"
                      className="input-number-box"
                      style={{ width: '100%' }}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem', marginTop: '0.2rem' }}>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-secondary)' }}>How will you fund this?</span>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                    {[
                      { label: '🏦 Use Available Savings (Deduct from liquid assets)', value: 'savings' },
                      { label: relationshipType === 'married' ? '📈 Save Until Wedding (Extra savings targeted before event)' : '📈 Save Until Celebration (Extra savings targeted before event)', value: 'save_targeted' },
                ...
                {editingEvent.weddingFundingMethod === 'debt' && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem', padding: '0.65rem', background: 'rgba(255,255,255,0.01)', borderRadius: '6px', border: '1px solid var(--border-color)', marginTop: '0.25rem' }}>
                    <span style={{ fontSize: '0.7rem', fontWeight: 'bold', textTransform: 'uppercase', color: 'var(--text-tertiary)' }}>Financing Details</span>
                ...
                {Number(editingEvent.weddingCost || 0) > (userAssets + spouseAssets) && (
                  <div style={{ border: '1px solid var(--warning)', backgroundColor: 'var(--warning-light)', padding: '0.65rem 0.85rem', borderRadius: '6px', fontSize: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.15rem', marginTop: '0.25rem' }}>
                    ...
                    <span>
                      Cost exceeds combined available savings by <strong>{formatCurrency(Number(editingEvent.weddingCost || 0) - (userAssets + spouseAssets))}</strong>.
                    </span>
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>
                      {(editingEvent.weddingFundingMethod || 'savings') === 'savings' && 'Note: This gap will result in negative liquid assets at the event age.'}
                      {(editingEvent.weddingFundingMethod || 'savings') === 'save_targeted' && 'Note: You will need to save this difference before the event.'}
                      {(editingEvent.weddingFundingMethod || 'savings') === 'debt' && `Financing adds ${formatCurrency(Number(editingEvent.weddingCost || 0) - (userAssets + spouseAssets))} of debt. Your net worth may go negative until the debt is paid down.`}
                    </span>
                    {editingEvent.weddingFundingMethod === 'debt' && isNetWorthBelowZero && (
                      <span style={{ fontSize: '0.7rem', color: 'var(--warning)', fontWeight: 'bold', marginTop: '0.15rem' }}>
                        Your net worth is below $0 because the celebration debt is larger than your available assets.
                      </span>
                    )}
                  </div>
                )}
        ...
        {stepId === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            <div style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
              ...
              <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', margin: 0 }}>
                {editingEvent.livingTogether !== false 
            ...
            {/* Savings Breakdown */}
            {editingEvent.livingTogether !== false && estimates && estimates.savingsBreakdown && (
        ...
        {/* STEP 4: RELATIONSHIP IMPACT */}
        {stepId === 4 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            <div style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '0.5rem' }}>
              <h4 style={{ fontSize: '0.95rem', fontWeight: 'bold', color: 'var(--text-primary)', margin: '0 0 0.25rem 0' }}>
                {relationshipType === 'married' ? 'Marriage Impact' : 'Relationship Impact'}
              </h4>
              <p style={{ fontSize: '0.78rem', color: 'var(--text-secondary)', margin: 0 }}>
                Compare your household financials and retirement readiness before and after this event.
              </p>
            </div>

            {/* Tax Filing Status - only shown for marriage type and if taxes are enabled */}
            {showTaxesStep && relationshipType === 'married' && (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              <div style={{ border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)', padding: '0.85rem', background: 'var(--bg-tertiary)' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: 'var(--text-secondary)', display: 'block', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.35rem', marginBottom: '0.5rem' }}>
                  {relationshipType === 'married' ? 'Before Marriage' : 'Before Event'}
                </span>
              ...
              <div style={{ border: '1px solid var(--primary)', borderRadius: 'var(--radius-md)', padding: '0.85rem', background: 'linear-gradient(135deg, rgba(99, 102, 241, 0.04) 0%, rgba(139, 92, 246, 0.04) 100%)' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: 'var(--primary)', display: 'block', borderBottom: '1px solid var(--primary)', paddingBottom: '0.35rem', marginBottom: '0.5rem' }}>
                  {relationshipType === 'married' ? 'After Marriage' : 'After Event'}
                </span>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', fontSize: '0.78rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--text-secondary)' }}>Combined Income:</span>
                    <strong style={{ color: 'var(--text-primary)' }}>{formatCurrency((editingEvent.combineFinances !== false ? combinedIncome : userIncome) / 12)}/mo</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--text-secondary)' }}>Combined Savings:</span>
                    <strong style={{ color: 'var(--success)' }}>{formatCurrency(editingEvent.combineFinances !== false ? combinedSavings : userSavings)}/mo</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--text-secondary)' }}>Combined Spending:</span>
                    <strong style={{ color: 'var(--danger)' }}>{formatCurrency((editingEvent.combineFinances !== false ? combinedSpendingVal : userSpendingPreRetirement) / 12)}/mo</strong>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <span style={{ color: 'var(--text-secondary)' }}>Savings Rate:</span>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px dashed var(--primary)', paddingTop: '0.4rem', marginTop: '0.1rem' }}>
                    ...
                    <strong style={{ color: 'var(--success)' }}>
                      {formatCurrency((editingEvent.combineFinances !== false ? combinedAssets : userAssets) - (editingEvent.combineFinances !== false ? combinedDebt : userDebt))}
                    </strong>
                  </div>
                </div>
              </div>
            ...
            {/* Celebration details summary */}
            {!isShortFlow && editingEvent.includeWeddingCost && (
              <div style={{ background: 'rgba(255,255,255,0.02)', padding: '0.75rem 0.85rem', borderRadius: '6px', border: '1px solid var(--border-color)', fontSize: '0.78rem', display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <strong style={{ color: 'var(--text-primary)' }}>
                    ...
                    {(editingEvent.weddingFundingMethod === 'save_targeted') && (relationshipType === 'married' ? 'Save Until Wedding' : 'Save Until Celebration')}
                ...
                {editingEvent.weddingFundingMethod === 'debt' && Number(editingEvent.weddingCost || 0) > (userAssets + spouseAssets) && (
                  <>
                    ...
                    <div style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', marginTop: '0.15rem' }}>
                      Financing adds {formatCurrency(Number(editingEvent.weddingCost || 0) - (userAssets + spouseAssets))} of debt. Your net worth may go negative until the debt is paid down.
                    </div>
                  </>
                )}
                {isNetWorthBelowZero && (
            ...
            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', textAlign: 'center', marginTop: '-0.5rem' }}>
              Retirement target calculated at <strong>{inputs.swr || 4.0}% SWR</strong> supporting both partners.
            </div>

            {/* Warnings & Confirmations */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
              {/* 1. Low Combined Spending Warning */}
              {editingEvent.combineFinances !== false && combinedSpendingVal <= userSpendingPreRetirement && (
                <div style={{ border: '1px solid var(--danger)', backgroundColor: 'var(--danger-light)', padding: '0.75rem', borderRadius: '6px', display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                  ...
                  <label htmlFor="confirm-zero-spending-preview" style={{ fontSize: '0.75rem', color: 'var(--text-primary)', cursor: 'pointer', margin: 0 }}>
                    ...
                    I confirm that combined household spending is less than or equal to my single spending (meaning my partner has no additional spending needs).
                  </label>
                </div>
              )}

              {/* 2. Zero Partner Personal Spending Warning */}
              {editingEvent.combineFinances !== false && partnerPersonalSpending === 0 && (
                <div style={{ border: '1px solid var(--danger)', backgroundColor: 'var(--danger-light)', padding: '0.75rem', borderRadius: '6px', display: 'flex', gap: '0.5rem', alignItems: 'flex-start' }}>
                  <input
                    type="checkbox"
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '1rem', borderTop: '1px solid var(--border-color)', paddingTop: '1rem', width: '100%' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            ...
            {stepId < 4 ? (
              ...
              >
                {relationshipType === 'married' ? 'Save Marriage Event' : 
                 relationshipType === 'domestic_partnership' ? 'Save Domestic Partnership' : 
