...
export function normalizeInputsStage(rawInputs) {
  ...
  if (!hasActiveChild) {
    incomeList = incomeList.filter(inc => !inc.id.startsWith('simple-inc-childcare') && !inc.id.startsWith('simple-inc-prechild'));
  }
  if (!hasActiveChild) {
    spendingPhases = spendingPhases.filter(p => !p.id.startsWith('simple-spend-childcare') && !p.id.startsWith('simple-spend-prechild'));
  }
