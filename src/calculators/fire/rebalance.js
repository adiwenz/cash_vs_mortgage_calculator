...
export function getSimulationValidationForPrice(price, level, inputs, buyHouseEv, baselineReadyAge, originalInputs, baselinePhases, baselineResults) {
  ...
  if (buyHouseEventIndex !== -1) {
    ...
    const simulatedEvent = tempInputs.lifeEvents[buyHouseEventIndex];
    if (simulatedEvent && simulatedEvent.homePrice !== price) {
      throw new Error(`Simulation validation price mismatch: expected event price to be ${price} but got ${simulatedEvent.homePrice}`);
    }
  }
...
function solveBisectionHomeValue(level, inputs, buyHouseEv, baselineReadyAge, baselineResults) {
  ...
  if (level === 'conservative') {
    ...
    const savings_capacity = 0;
    total_housing_pool = Math.max(0, current_housing + Math.max(0, current_surplus) + wants_capacity + savings_capacity - emergencySurplusFloor);
  } else if (level === 'stretch' || level === 'aggressive') {
    const wants_capacity = Math.max(0, current_wants - wants_floor);
  ...
  // Calculate nominal active Social Security / other retirement income at target_retirement_age
...
export function getRebalanceStrategies(inputs, activeBuyHouseEv, baselineReadyAge) {
  ...
  // Run bisection method optimizer for conservative, balanced, and stretch/aggressive strategies
