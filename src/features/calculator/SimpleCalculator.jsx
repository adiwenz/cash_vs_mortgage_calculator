...
export default function SimpleCalculator() {
  ...
  const ticks = useMemo(() => {
    let min = 0;
    let max = typeof yAxisMax === 'number' ? yAxisMax : 100000;
    if (chartData && chartData.length && lineKeys && lineKeys.length) {
      chartData.forEach((row) => {
        lineKeys.forEach((lk) => {
          const val = row[lk.key];
          if (val !== undefined) {
            if (val < min) min = val;
            if (val > max) max = val;
          }
        ...
        {errors.length > 0 ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '1rem' }}>
              <div style={{ background: 'rgba(30, 58, 95, 0.04)', border: '1px solid rgba(30, 58, 95, 0.15)', borderRadius: '10px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                <span style={{ fontSize: '0.85rem', fontWeight: '700', color: '#1e3a5f', display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                  ⏳ Opportunity Cost
                </span>
